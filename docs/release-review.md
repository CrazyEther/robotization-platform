...
- SimPy: 8 тестов, включая детерминизм, ограничение грузоподъёмности, батарею и непроходимый маршрут.
- Playwright: 10 сквозных проверок desktop/mobile с реально работающим Python API.
- Скриншоты реальной программы: `docs/design/ris-*.png`.

**Текущая ревизия:** `simpy-transport/0.3` различает доставку и возврат, учитывает частичный пробег/энергию и возвращает P95, среднее время задания и загрузку зарядных постов. Защита от ответа устаревшей симуляции проверена браузерным тестом; сквозной экспорт проверяет связь технических результатов с экономикой. Эти проверки не являются валидацией по реальному предприятию.
