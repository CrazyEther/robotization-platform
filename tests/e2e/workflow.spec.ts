...
test('manufacturer EFEU geometry renders in WebGL',async({page},info)=>{
 ...
 await page.goto('/');await page.getByRole('button',{name:'Оценить роботизацию'}).click();await page.getByRole('navigation',{name:'Шаги оценки'}).getByRole('button').nth(3).click();
...
test('client report download and spatial data validation',async({page})=>{
 await page.goto('/');await page.getByRole('button',{name:'Оценить роботизацию'}).click();
...
test('real catalog → comparison → honest incomplete economy → event gate',async({page})=>{
 const errors:string[]=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('/');await page.getByRole('button',{name:'Оценить роботизацию'}).click();
...
test('research, filter, detail provenance and local empty scenario round-trip',async({page})=>{
 await page.goto('/');await page.getByRole('button',{name:'Оценить роботизацию'}).click();await page.getByRole('button',{name:/Каталог решений/}).click();await page.getByRole('textbox',{name:'Поиск решений'}).fill('KIRA');await expect(page.locator('.product-card')).toHaveCount(1);
